      ...
      <section className="bg-cream px-6 py-14 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <h2 className="text-center font-heading text-3xl font-light tracking-tight text-navy md:text-4xl">
            {t("advisory.flow.title")}
          </h2>
          <p className="mx-auto mt-4 max-w-2xl text-center leading-relaxed text-text-muted">
            {t("advisory.flow.description")}
          </p>
          {/* Horizontal flow */}
          <div className="mt-10 flex flex-col items-center gap-3 md:flex-row md:justify-center md:gap-0">
            {(tArray("advisory.flow.steps") as string[]).map((step, i, arr) => (
              <div key={i} className="flex items-center gap-3">
                <div className="flex items-center gap-3 rounded-lg border border-border-warm bg-white px-5 py-3">
                ...
                {i < arr.length - 1 && (
                  <>
                    <svg className="hidden h-5 w-5 text-gold md:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
                    </svg>
                    <svg className="h-5 w-5 text-gold md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <p className="mt-6 text-center text-sm italic text-gold">
            {t("advisory.flow.callout")}
          </p>
        </div>
      </section>
