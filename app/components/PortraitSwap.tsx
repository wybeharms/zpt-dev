  ...
  return (
    ...
    >
      {/* Professional half — default visible. Fades out on hover (desktop)
          OR when `toggled` is true (touch tap). The clip-path crops a 1px
          sliver off the right edge so the seam at the center of the 3:2
          composite never appears as a hairline. 1px is the minimum that
          still hides the seam; anything larger exposes a visible strip of
          the parent background on the right edge. */}
      <img
        src={src}
        alt={alt}
        className={`absolute inset-0 h-full w-full object-cover object-left [clip-path:inset(0_1px_0_0)] transition-opacity duration-[350ms] ease-out group-hover:opacity-0 ${
          toggled ? "opacity-0" : "opacity-100"
        }`}
      />
      {/* Marine half — fades in on hover OR when toggled. Same 1px clip,
          mirrored to the left edge for the same reason. */}
      <img
        src={src}
        alt=""
        aria-hidden="true"
        className={`absolute inset-0 h-full w-full object-cover object-right [clip-path:inset(0_0_0_1px)] transition-opacity duration-[350ms] ease-out group-hover:opacity-100 ${
